...
impl Cluster {
    pub fn init(id: usize, points: Arc<Array<f64,Ix2>>, point_id: usize) -> Cluster {
    ...
    pub fn radius(&self) -> f64 {
        let radius = self.members.iter().map(|x| length((&self.array.row(*x) - &self.center).view())).sum::<f64>() / self.weight as f64;
        // eprintln!("R:{:?}",radius);
        radius
    }
